...
wss.on("connection",ws=>{
  ...
  ws.on("message",raw=>{let m;try{m=JSON.parse(raw)}catch{return}
    ...
    if(m.type==="join"){
      ...
      if(ws.roomId&&rooms.get(ws.roomId)){const old=rooms.get(ws.roomId);old.clients.delete(ws);old.broadcasters.delete(ws);if(old.broadcasters.size===0){} }
    ...
    if(m.type==="stream-stopped"){
      if(ws.role==="broadcaster")ws.streaming=false;
