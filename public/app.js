...
const voicePeers=new Map(),screenOut=new Map(),screenIn=new Map(),iceQueue=new Map();
const iceKey=(channel,id)=>`${channel}:${id}`;
function queueIce(channel,id,c){const k=iceKey(channel,id);if(!iceQueue.has(k))iceQueue.set(k,[]);iceQueue.get(k).push(c)}
async function flushIce(channel,id,pc){const k=iceKey(channel,id);for(const c of iceQueue.get(k)||[])await pc.addIceCandidate(c);iceQueue.delete(k)}
...
function removeScreen(sourceId){screenIn.get(sourceId)?.close();screenIn.delete(sourceId);document.querySelector(`video.remoteStream[data-source="${sourceId}"]`)?.remove();if(!$('streamsGrid').querySelector('.remoteStream'))$('empty').classList.remove('hidden')}

async function makeScreenPeer(targetId){
  if(!screenStream||screenOut.has(targetId))return;
  const pc=new RTCPeerConnection(cfg());screenOut.set(targetId,pc);
  screenStream.getTracks().forEach(t=>pc.addTrack(t,screenStream));
  pc.onicecandidate=e=>e.candidate&&send({type:'candidate',channel:'screen',target:targetId,candidate:e.candidate});
  pc.onconnectionstatechange=()=>{if(['failed','closed','disconnected'].includes(pc.connectionState)){pc.close();screenOut.delete(targetId)}};
...
function stopScreen(){
  ...
  screenOut.forEach(p=>p.close());screenOut.clear();screenIn.forEach(p=>p.close());screenIn.clear();
...
async function handle(m){
  ...
  if(m.type==='offer'&&m.channel==='screen'){
    let pc=screenIn.get(m.from);
    if(!pc){pc=new RTCPeerConnection(cfg());screenIn.set(m.from,pc);pc.ontrack=e=>{const v=createScreenElement(m.from,'Transmissão de '+m.from);v.srcObject=e.streams[0];v.muted=false;v.play().catch(()=>{});$('empty').classList.add('hidden')};pc.onicecandidate=e=>e.candidate&&send({type:'candidate',channel:'screen',target:m.from,candidate:e.candidate});pc.onconnectionstatechange=()=>{if(['failed','closed','disconnected'].includes(pc.connectionState))removeScreen(m.from)}}
    await pc.setRemoteDescription(m.offer);await flushIce('screen',m.from,pc);const ans=await pc.createAnswer();await pc.setLocalDescription(ans);send({type:'answer',channel:'screen',target:m.from,answer:ans});return;
  }
  if(m.type==='offer'&&m.channel==='voice'){
    ...
    await pc.setRemoteDescription(m.offer);await flushIce('screen',m.from,pc);const ans=await pc.createAnswer();await pc.setLocalDescription(ans);send({type:'answer',channel:'voice',target:m.from,answer:ans});return;
  }
  if(m.type==='answer'){const pc=m.channel==='voice'?voicePeers.get(m.from):screenOut.get(m.from);if(pc){await pc.setRemoteDescription(m.answer);await flushIce(m.channel,m.from,pc)}return}
  if(m.type==='candidate'){const pc=m.channel==='voice'?(voicePeers.get(m.from)):((screenIn.get(m.from))||(screenOut.get(m.from)));if(pc?.remoteDescription)await pc.addIceCandidate(m.candidate);else queueIce(m.channel,m.from,m.candidate);return}
}
