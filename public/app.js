...
$('broadcast').onclick=async()=>{if(!room)return alert('Entre em uma sala primeiro.');if(screenStream)return alert('Você já está transmitindo.');if(role==='broadcaster')return startScreen();send({type:'join',roomId:room.id,password:'',role:'broadcaster'})};
...
async function startScreen(){
  try{
    const h=+$('quality').value,f=+$('fps').value;
    ...
    screenStream=await navigator.mediaDevices.getDisplayMedia({video:{width:{ideal:Math.round(h*16/9)},height:{ideal:h},frameRate:{ideal:f,max:f}},audio:false});
  ...
  }catch(e){screenStream=null}
}

function createScreenElement(sourceId,label){
  let v=document.querySelector(`video.remoteStream[data-source="${sourceId}"]`);
  if(v)return v;
  v=document.createElement('video');v.className='remoteStream';v.dataset.source=sourceId;v.autoplay=true;v.playsInline=true;v.controls=true;v.setAttribute('aria-label',label||'Transmissão');
...
async function handle(m){
  ...
  if(m.type==='joined'){
    ...
    if(role==='broadcaster' && !screenStream)await startScreen();return;
  }
  ...
  if(m.type==='peer-for-stream'&&role==='broadcaster'&&screenStream)return makeScreenPeer(m.peerId);
  ...
  if(m.type==='offer'&&m.channel==='screen'){
    ...
    if(!pc){pc=new RTCPeerConnection(cfg());screenPeers.set(m.from,pc);pc.ontrack=e=>{const v=createScreenElement(m.from,'Transmissão de '+m.from);v.srcObject=e.streams[0];$('empty').classList.add('hidden')};pc.onicecandidate=e=>e.candidate&&send({type:'candidate',channel:'screen',target:m.from,candidate:e.candidate});pc.onconnectionstatechange=()=>{if(['failed','closed','disconnected'].includes(pc.connectionState))removeScreen(m.from)}}
