...
async function makeScreenPeer(targetId){
  ...
  for(const sender of pc.getSenders()){if(sender.track?.kind==='video'){const p=sender.getParameters();p.encodings=p.encodings?.length?p.encodings:[{}];p.encodings[0].maxBitrate=state.screen?.profile?.bitrate;p.encodings[0].maxFramerate=state.screen?.profile?.fps;p.encodings[0].priority='high';try{await sender.setParameters(p)}catch{}}}
...
async function handle(m){
  ...
  if(m.type==='answer'){const map=m.channel==='voice'?voicePeers:screenOut,pc=map.get(m.from);if(pc){await pc.setRemoteDescription(m.answer);for(const c of iceQueue.get(m.from)||[])await pc.addIceCandidate(c);iceQueue.delete(m.from)}return}
  if(m.type==='candidate'){const map=m.channel==='voice'?voicePeers:screenIn,pc=map.get(m.from)||screenOut.get(m.from);if(pc?.remoteDescription)await pc.addIceCandidate(m.candidate);else{if(!iceQueue.has(m.from))iceQueue.set(m.from,[]);iceQueue.get(m.from).push(m.candidate)}}
}
